...
ALTER TABLE public.rooms ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'available' CHECK (status IN ('available', 'occupied', 'maintenance', 'out_of_order'));
ALTER TABLE public.rooms ADD COLUMN IF NOT EXISTS inventory_count INTEGER DEFAULT 1;

-- Update bookings table with more fields for admin management
ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS guest_count INTEGER DEFAULT 1;
ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS admin_notes TEXT;
ALTER TABLE public.bookings ADD COLUMN IF NOT EXISTS total_amount DECIMAL;
...
-- Insert sample pricing data (with correct enum values)
INSERT INTO public.room_pricing (room_type_id, base_price, start_date, end_date)
SELECT id, 
       CASE 
         WHEN type = 'standard' THEN 150.00
         WHEN type = 'deluxe' THEN 250.00
         WHEN type = 'suite' THEN 450.00
         ELSE 200.00
       END,
       CURRENT_DATE,
